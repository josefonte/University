...
func main() {
	...
	connection.Close()
	if Type == "content" && Subtype == "content_sub" && Data == "OK" {
		//start streaming process
		fmt.Println("My addr:", node_ip)
		source_add, err := net.ResolveUDPAddr("udp", node_ip+":"+pkgs.UDP_PORT) //st.Source.IP+
		if err != nil {
			panic(err)
		}

		conn, err := net.ListenUDP("udp", source_add)
		if err != nil {
			panic(err)
		}
		defer conn.Close()
		...
		buffer := make([]byte, 188*10) // Buffer to read UDP data
		for {
