...
func MessageBuilder(Type string, SubType string, data []byte) []byte {
	...
	} else if strings.ToLower(Type) == "content" {
		typeBytes[0] = 0b00000011
		if strings.ToLower(SubType) == "content_sub" {
			typeBytes[1] = 0b00000001
		} else if strings.ToLower(SubType) == "content_term" {
			typeBytes[1] = 0b00000010
		} else if strings.ToLower(SubType) == "videos_catalog" {
			typeBytes[1] = 0b00000011
		}
	}
...
func MessageDecoder(buf []byte, n int) (Type string, Subtype string, Data string, err error) {
	...
	case (buf[0]) == 0b00000011: //content messages
		Type = "content"
		if (buf[1]) == 0b00000001 {
			Subtype = "content_sub"
		} else if (buf[1]) == 0b00000010 {
			Subtype = "content_term"
		} else if (buf[1]) == 0b00000011 {
			Subtype = "videos_catalog"
		}
	default:
